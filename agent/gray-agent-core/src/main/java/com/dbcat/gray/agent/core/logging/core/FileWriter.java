...
package com.dbcat.gray.agent.core.logging.core;
    ...
    private void switchFile() {
        if (fileSize > Config.Logging.MAX_FILE_SIZE) {
            ...
            forceExecute(new Callable() {
                @Override
                public Object call() throws Exception {
                    new File(Config.Logging.DIR, Config.Logging.FILE_NAME).renameTo(new File(Config.Logging.DIR, Config.Logging.FILE_NAME + new SimpleDateFormat(".yyMMddHHmm")
                            .format(new Date())));
                    return null;
                }
            });
